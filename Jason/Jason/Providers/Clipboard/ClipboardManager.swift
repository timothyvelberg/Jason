...
class ClipboardManager: ObservableObject {
    ...
    private func checkForChanges() {
        ...
        // Respect pasteboard privacy hints: password managers and secure input
        // fields mark copied secrets with these types precisely so clipboard
        // managers skip them. Never capture or persist such content.
        let concealedTypes: Set<NSPasteboard.PasteboardType> = [
            NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType"),
            NSPasteboard.PasteboardType("org.nspasteboard.TransientType"),
            NSPasteboard.PasteboardType("org.nspasteboard.AutoGeneratedType")
        ]
        if let types = pasteboard.types, !concealedTypes.isDisjoint(with: types) {
            print("[ClipboardManager] Skipping concealed/transient clipboard content")
            return
        }
